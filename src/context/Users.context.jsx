...
async function addVehicle(addCar){
  await axios.vehicle(BASEURL+"/fleet",addCar)
  dispatch({type:'ADD_VEHICLE', payload:addCar})
}

async function login(mail,pwd){
  const res = await axios.vehicle(BASEURL+"/login", {email:mail, password:pwd})
...
dispatch({type: 'GET_SINGLE_VEHICLE', payload: res.data})
}
...
return (
  <VehicleContext.Provider value={{vehicles: state.vehicles, user:state.user, IndividualVehicle:state.IndividualVehicle, addVehicle,login,signup,changeName,getByID,deleteVehicle,cerrarSesion}}>
    {{children}}
